...
6. `model.eval()`、`with torch.no_grad()`与`requires_grad=False`的对比
   ```python
   model.eval() # 关闭随机失活，关闭BatchNorm，但是没进行参数冻结
   with torch.no_grad():  # 关闭梯度计算，省显存、提速
   requires_grad=False # 参数冻结
   ```
   - 表格对比

    |          功能           |  model.eval()  |  with torch.no_grad()  |  requires_grad=False  |
    |:---------------------:|:--------------:|:----------------------:|:---------------------:|
    |      关闭 Dropout       |       ✅        |           ❌            |           ❌           |
    |     固定 BatchNorm      |       ✅        |           ❌            |           ❌           |
    |        关闭梯度计算         |       ❌        |           ✅            |           ✅           |
    |         节省显存          |       ❌        |           ✅            |          少量           |
    |        冻结参数不更新        |       ❌        |           ❌            |           ✅           |
    |         作用范围          |      模型模式      |         代码块内临时         |         参数永久          |
    |         主要用途          |      推理模式      |          推理加速          |         冻结微调          |
   - 概述
     - `model.eval()`：切换模型从训练模式转为评估推理模式，仅改变网络层的运行逻辑，不影响梯度计算、不冻结参数
       - 关闭`Dropout`层的随机失活机制，所有神经元全部参与前向计算
       - 固定`BatchNorm`、`LayerNorm`等归一化层，不再使用当前批次实时统计均值方差，改用训练阶段固化的全局均值、方差
       - 仅对模型层行为生效，不改变参数可训练属性
       - 全局持久生效，直到调用`model.train()`才会恢复训练模式
     - `with torch.no_grad()`：上下文管理器，临时禁用梯度计算与计算图构建，不保存反向传播所需信息
       - 代码块内不构建计算图、不记录张量梯度，大幅节省显存、加快前向推理速度
       - 不改变`Dropout`、`BatchNorm`层行为，也不修改模型参数的`requires_grad`属性
       - 仅为临时生效，退出**代码块**后自动恢复正常梯度记录。
     - `requires_grad=False`：设置模型参数禁止计算梯度，从根源上冻结权重，让优化器无法更新该部分参数
       - 被设置的参数不再参与反向传播梯度求解
       - 优化器`optimizer.step()`不会对该类参数做权重更新（只是不更新了，但是不代表相关缓存矩阵等不占用显存、不计算）
       - 不改变网络层运行模式，不自带省显存效果
       - 半永久持久生效，需手动改回`requires_grad=True`才可解冻。
