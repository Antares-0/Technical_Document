   ...
   3. 稀疏自注意力机制（`GPT-3`）（稀疏注意力机制并非`GPT-3`原创）
      ...
      - 稀疏自注意力机制：`GPT-3`不再使用当前词元计算和其他所有词元之间的相关性，以此在能够保证训练效果的基础上适当降低训练成本
        ...
        - 稀疏自注意力机制详解
          ...
          - 数学描述：
            - $X$，模型的输入；$S$，稀疏自注意力关注的序列
              $$
              \text{Attend}(X,S) = \left(a(x_i,S_i)\right)
              $$
              $$
              a(x_i,S_i) = \text{softmax}\left(
              \frac{(W_q x_i) K_{S_i}^\top}{\sqrt{d}}
              \right) V_{S_i}
              $$
              $$
              K_{S_i} = (W_k x_j)_{j\in S_i}, \quad
...
2. `Llama1`模型结构与`Transformer`区别
   ...
   - 训练优化：使用了大量的技术进行训练优化
     ...
     - 模型并行、序列并行
     - 检查点机制
3. `Llama2`模型结构与`Llama1`区别
   - 分组查询注意力
4. `Llama2-chat`模型与`Instruct GPT`
   - 有用性奖励模型
   - 安全性奖励模型
   - 拒绝采样：扩充数据集
5. 
...
参考资料：
1. 稀疏注意力机制：https://arxiv.org/pdf/1904.10509
