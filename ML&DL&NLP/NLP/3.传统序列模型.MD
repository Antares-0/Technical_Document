...
2. LSTM结构介绍
   ...
   - 隐藏状态的参数更新
     ...
     - 隐藏状态的计算公式：
       ...
       $$
3. LSTM是怎么缓解梯度消失和梯度爆炸的？一句话结论：通过引入记忆单元，部分地保存了相对较为遥远一点的数据（存在一条相对稳定的通道，能够将遥远处位置的信息最终传递给输出）
...
8. 存在的问题：尽管LSTM相较传统RNN解决了长期依赖问题，性能大幅提升，但在实际应用中，仍存在一些明显的局限性和问题，主要包括：
   - 难以并行计算：LSTM的时间步之间具有强依赖性（后一个时间步的输入依赖前一个时间步的输出），导致无法进行大规模并行加速，训练和推理速度受限。
   - 参数量大，计算开销高：每个LSTM单元内部包含多个门控机制（输入门、遗忘门、输出门），每个门都需要独立计算，导致参数数量和计算量远大于普通RNN。在资源受限的场景下（如移动端、嵌入式设备），部署LSTM会面临挑战。
   - 长期依赖建模仍然有限：虽然LSTM延缓了梯度消失问题，但并不能完全消除。当序列极长时，模型依然难以有效捕捉非常远距离的依赖关系。

## 三、GRU
1. GRU为什么会出现？基于LSTM存在的“参数量大”问题作了适当的优化，其他问题依然没有解决
2. GRU定义：Gated Recurrent Unit（GRU）是为了进一步简化LSTM结构、降低计算成本而提出的一种变体。GRU保留了门控机制的核心思想，但相比LSTM，结构更为简洁，参数更少，训练效率更高。在许多实际任务中，GRU能在保持类似性能的同时，显著减少训练时间。
3. GRU结构
   
   ![](./fig/gru.png)
   - GRU引入两个门控单元——重置门和更新门，同时引入一个隐藏状态，还有引入一个候选隐藏状态，来控制最终的隐藏状态输出结果
   - 重置门：决定遗忘过去的多少信息
     - 设计思想：决定遗忘多少过去的信息，就根据当前的输入和上一个时间步的隐藏状态共同决定
     - 计算公式
       $$
       r_t = \sigma\left( h_{t-1} \cdot W_h^r + x_t \cdot W_x^r + b_r \right)
       $$
       - $r_t$：t 时刻的重置门输出，取值范围 $[0,1]$
       - $\sigma$：Sigmoid 激活函数，用于将输出约束在 $0 \sim 1$ 区间
       - $h_{t-1}$：t-1 时刻的隐藏状态
       - $W_h^r$：重置门中对应**前一时刻隐藏状态**的权重矩阵
       - $x_t$：t 时刻的输入特征
       - $W_x^r$：重置门中对应**当前时刻输入**的权重矩阵
       - $b_r$：重置门的偏置项
     - 重置门计算图
       
       ![](./fig/gruregate.png)
   - 更新门：决定本时间步内需要记住多少信息
     - 设计思想：决定本轮需要记住多少过去的信息，必然依赖当前的输入和上一个时间步的隐藏状态共同决定
     - 计算公式：
       $$
       z_t = \sigma\left( h_{t-1} \cdot W_h^z + x_t \cdot W_x^z + b_z \right)
       $$
       - $z_t$：t 时刻的更新门输出，取值范围 $[0,1]$
       - $\sigma$：Sigmoid 激活函数，用于将输出约束在 $0 \sim 1$ 区间
       - $h_{t-1}$：t-1 时刻的隐藏状态
       - $W_h^z$：更新门中对应**前一时刻隐藏状态**的权重矩阵
       - $x_t$：t 时刻的输入特征
       - $W_x^z$：更新门中对应**当前时刻输入**的权重矩阵
       - $b_z$：更新门的偏置项
     - 更新门计算图
     
       ![](./fig/gruupdate.png)
   - 候选隐藏状态：隐藏状态的中间态，需要经过各种门的联合计算得到最终隐藏状态的值
     - 设计思想：隐藏状态需要根据上一时刻的隐藏状态和当前输入决定（但是计算方式当然要有所不同），同时还要根据重置门的输出共同决定
     - 计算公式：
       $$
       \widetilde{h}_t = \tanh\left( \left( r_t \odot h_{t-1} \right) W_h + x_t W_x + b \right)
       $$
       - $\widetilde{h}_t$：当前时间步的**候选隐藏状态** 
       - $\tanh$：双曲正切激活函数，将输出映射到 $[-1, 1]$ 区间
       - $r_t$：重置门（Reset Gate），控制历史信息的保留程度
       - $\odot$：按元素乘法（Hadamard 乘积）
       - $h_{t-1}$：上一时间步的隐藏状态
       - $W_h$：历史隐藏状态对应的权重矩阵
       - $x_t$：当前时间步的输入向量
       - $W_x$：输入向量对应的权重矩阵
       - $b$：偏置向量
   - 隐藏状态
     - 设计思想：隐藏状态需要结合候选隐藏状态和更新门来决定，因为需要结合更新门来确定下一个时间步的隐藏状态是什么
     - 计算公式：
       $$
       ...
       - $h_t$：t 时刻的最终隐藏状态
       - $z_t$：t 时刻的更新门输出（取值范围 $[0,1]$）
       - $\odot$：哈达玛积（元素级相乘），控制对应元素的加权融合
       - $h_{t-1}$：t-1 时刻的原始隐藏状态
       - ${1}$：全 1 向量（与 $z_t$ 维度一致），用于计算补数
       - $\widetilde{h}_t$：t 时刻的候选隐藏状态
4. GRU与梯度消失
   - GRU缓解了RNN的梯度消失问题，因为存在一条信息通路，能够将遥远处的信息传递给最终的输出，这条路径上的梯度变化相对较为平缓
   - GRU依然不能完全解决RNN的梯度消失问题，门的计算值依然是小于1的，导致远距离状态下还是要梯度消失的
5. 双层多向结构与LSTM、RNN几乎完全一致
6. 代码API（可关注[网站](https://pytorch.org/docs/stable/generated/torch.nn.gru.html)）
   - 创建GRU对象
     ```python
     torch.nn.GRU(
        input_size,  # 每个时间步输入特征的维度（词向量维度）
...
## 四、三种传统序列模型的对比（基于项目）
1. RNN的收敛结果不好，存在大幅震荡的情况
2. LSTM和GRU的收敛效果不错，其中GRU的收敛速度更快
3. RNN的准确性一般，因为它的梯度消失问题更严重
