...
6. `Instruct GPT`：详情见[RLHF](../../LLM/ReinforcementLearning/04.RLHF-基于人类反馈的强化学习.MD)
...
2. `Llama`系列的贡献
   - `Llama1`的贡献
     - 推翻了“唯参数论”，确立了**小模型+大数据**的开发范式
     - 公开数据集实现了SOTA效果，打破数据垄断
     - 模型结构修改
     - 使用了非常多的评估基线，提供了广泛的性能评估手段
   - `Llama2`的贡献（主要是`Llama2-chat`）
     - 双奖励模型
     - 拒绝采样策略
   - `Llama3`的贡献
     - 直接偏好优化
3. `Llama1`模型结构与`Transformer`区别
   - 只保留解码器部分
   - pre-Norm，归一化方案也变成了RMSNorm
   - SwiGLU激活函数，与GeLU共治天下
   - RoPE位置编码（旋转位置编码）
   - 训练优化：使用了大量的技术进行训练优化
     - 与FlashAttention类似的xformers技术来优化
     - 模型并行、序列并行
     - 检查点机制

   ![](./fig/llamaVStransformer.png)
4. `Llama2`阶段同时发布了两个模型，一个是预训练模型`Llama2`，另一个是基于前者进行微调的对话模型`Llama2-chat`
   - 预训练模型`Llama2`
     - 数据集优化，更严格的数据清洗流程、更合理的数据混合方案、更大的训练`token`量
     - 原始的多头注意力机制（MHA，Multi-Head Attention）改成分组查询注意力机制（GQA，Grouped-Query Attention）
   - 微调的对话模型`Llama2-chat`
     - 【创新一】存在两个奖励模型：有用性奖励模型（评估输出是否有用），安全性奖励模型（评估输出是否安全，是否违背公序良俗）
     - 【创新二】引入拒绝采样：使用某一个模型统一生成K个回复，使用奖励模型进行打分，取前分数最高的一部分来构造数据集，其他样本拒绝采纳
     - 完整训练流程如下：
       - 预训练
       - 监督微调
       - 基于监督微调的模型训练安全性奖励模型和有用性奖励模型（人工标注的数据来训练）
       - 通过奖励模型对70B SFT模型进行拒绝采样
       - 基于PPO算法进行RLHF
       - 重复拒绝采样过程，迭代来提高模型性能

   ![](./fig/llama2chat.png)
5. `Llama3`模型简介
   - 设计理念：团队认为高质量基础模型的开发存在三个关键的影响因素——数据、规模与复杂度管理
     - 数据：数据体量和质量的双重提升
     - 规模：计算量和模型参数量提升，上下文长度提升
     - 复杂度管理：研究团队的目的不是追求理论上最前沿、最复杂的架构或算法，而是确保整个模型开发流程能够稳定、高效地扩展到巨大的规模
       - 没有采用花里胡哨的“混合专家架构”，采用了稳妥的稠密架构
       - 使用了直接偏好优化来实现强化学习对模型进行微调训练
   - `Llama3`预训练
     - 数据维度：数据质量、数据混合方案进一步改进 + 退火方案
     - 训练维度：AdamW、学习率衰减、长上下文预训练（渐进式）
     - 模型架构维度：根据缩放定律计算的模型大小、稠密Transformer架构、旋转位置编码频率提升以适应长上下文
   - `Llama3`后训练
     - 使用了直接偏好优化进行RLHF
     - 奖励模型收敛为一个
     - 完整流程
       - 基于收集的人类偏好数据训练奖励模型
       - 通过奖励模型对当前最优的模型评分，进行拒绝采样收集SFT数据
       - 进行监督微调
       - DPO训练，得到最终的DPO模型
       - 选取DPO训练过程中最优的检查点作为下一轮迭代的最优模型
       - 随着模型训练的进行，后训练阶段的数据也在不断收集，不断用新的数据训练模型，同时用最新的模型生成更多的数据。



-------
参考资料：
1. 稀疏注意力机制论文原文：https://arxiv.org/pdf/1904.10509
2. 旋转位置编码论文原文：https://arxiv.org/pdf/2104.09864v2
3. 稀疏混合专家模型实现：https://github.com/AviSoori1x/makeMoE/tree/main
4. MoE实现：https://github.com/deepseek-ai/DeepSeek-MoE
5. GQA实现：https://github.com/fkodom/grouped-query-attention-pytorch
6. Hugging Face实现RoPE：https://github.com/huggingface/transformers/blob/main/src/transformers/models/llama/modeling_llama.py#L214 ，查看 LlamaRotaryEmbedding 类
7. Hugging Face实现GQA：https://github.com/huggingface/transformers/blob/main/src/transformers/models/llama/modeling_llama.py#L214 ，查看 LlamaGroupedQueryAttention 类
