...
## 五、参数更新方法（公式参考./5.学习的技巧.MD）
1. Momentum动量法
   ```python
   # 加一个momentum=0.9就可以了，0.9给予一定的历史梯度权重
   optimizer = torch.optim.SGD( [X_clone], lr=lr, momentum=0.9)
   ```
2. 学习率衰减
   ```python
   # 1）等间隔衰减
   scheduler_lr = torch.optim.lr_scheduler.StepLR(optimizer, step_size, gamma)
   scheduler_lr.step()
   # optimizer：要实现学习率衰减的优化器
   # step_size：间隔，每迭代多少次，学习率就衰减gamma
   # gamma：衰减的比例
   
   # 2）指定间隔衰减
   scheduler_lr = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones, gamma)
   scheduler_lr.step()
   # optimizer：要实现学习率衰减的优化器
   # milestones：指定衰减的间隔，迭代次数到达就执行衰减  milestones=[10, 50, 200]
   # gamma：衰减的比例
   
   # 3）指数衰减
   scheduler_lr = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma)
   scheduler_lr.step()
   # optimizer：要实现学习率衰减的优化器
   # gamma：衰减的比例
   ```
...
4. RMSProp：均方根传播
   ...
   optimizer = RMSprop( [X_clone], lr=lr, alpha=0.9)
   ```
5. Adam：自适应矩估计
   ```python
...
## 六、应用案例：房价预测
1. 流程
   - 特征工程：虽然说神经网络会比机器学习降低对特征工程的依赖程度，但也不能说完全不做特征工程
     - 数值型：不需要特殊处理
     - 枚举类：独热编码
     - 某些字段不存在，可以删除这一行数据，也可以给一个缺省值（比如零），或者用全体数据的均值代替
   - 定义神经网络模型
     - 引入了BN正则化
   - 定义损失函数
     - 定义为相对误差，而不是绝对误差；使用对数来衡量
2. 代码细节见(./ML&DL&NLP/DL/code&data/chap7/house_price.py)
