...
3. 方案二：权值衰减（与方法三的主要区别是，直接作用在参数上，而不是梯度）
   - 方案：在模型学习过程中对大的权重进行「惩罚」，可有效抑制过拟合，这种方法被称为**权值衰减**。过拟合的常见诱因之一就是权重参数取值过大。
     $$\theta_{t} = (1-\eta\lambda)\theta_{t-1} - \eta g_t$$
   - 不影响梯度计算，属于“直接作用于参数”的正则方式
4. 方案三：L1正则化与L2正则化（与方法二的主要区别是，通过影响梯度计算间接影响参数更新）
   - 方案：在计算梯度时加入惩罚项，进而影响参数的更新
     $$Loss' = Loss + \frac{1}{2} \cdot \lambda \cdot \|W\|^2$$
     - 参数说明
       - $\|W\|$：权重 $W=(w_1, w_2, \dots, w_n)$ 的 L2 范数，计算公式为：$$\|W\| = \sqrt{w_1^2 + w_2^2 + \dots + w_n^2}$$
       - $\lambda$：控制正则化强度的超参数（$\lambda$ 越大，对大权重的惩罚越重）
   - 不直接操作参数，而是通过在损失函数中添加正则项，改变梯度的计算结果，再通过梯度下降间接影响参数更新，属于“通过梯度间接作用于参数”的正则方式。类似机器学习中的正则化操作
5. 方案四：Dropout随机失活、暂退法【激活函数层之后，线性层之前】
