...
5. 高级设计 High-level design
   ...
   4. 通知功能
      1. 使用消息队列，将需要通知的消息放入消息队列；消息队列将消息路由至对应设备，实现提示功能
      2. 安卓端推送通知功能的实现依赖于Firebase Cloud Messaging，苹果端推动通知功能的实现依赖于Apple Push Notification Service。这类服务的特点是应用程序接入后，由他们实现对用户设备的推送
   5. 已读回执功能
      1. 当用户打开消息聊天框后 or 通过pull拉取对话框消息时，标记该消息已读
      2. 或可以采用浏览上报等操作，当用户浏览到某些消息后，由端上执行上报功能
6. 细节设计 Detailed design
   1. 数据分区
      1. 解决方案：基于哈希算法的、基于列表的、基于范围的
      2. 数据倾斜问题：可以通过一致性哈希算法解决（加入虚拟节点）
   2. 对象存储：存储发送的文件，并提供访问服务（有ObjectStorage完整方案可以采用）
